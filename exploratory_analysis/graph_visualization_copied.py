...
class GraphVisualization:
    """Class for visualizing networkx graph
    """
    def __init__(
        self,
        nx_graph: nx.Graph,
        ...
        # check dimensions
        if all(len(pos.get(v, [])) == 2 for v in nx_graph):
            self.is_3d = False
        elif all(len(pos.get(v, [])) == 3 for v in nx_graph):
            self.is_3d = True
        else:
            raise ValueError
     ...
     def _get_setting(self, setting_name, edge=None):
        ...
        # setting is a constant
        if setting is not None and not callable(setting) and not isinstance(setting, dict):
            return setting

        if edge is None:  # vertex-specific
            if setting is None:  # default is used
                if callable(default_setting):  # default is a function
                    return [def_func(v) for v in self.nx_graph.nodes()]
                else:  # default is a constant
                    return default_setting
            elif callable(setting):  # setting is a function
                return [setting(v) for v in self.nx_graph.nodes()]
            elif isinstance(setting, dict):  # setting is a dict
                return [setting.get(v, def_func(v)) for v in self.nx_graph.nodes()]
        
        # edge-specific -> The case where edge is not None
        if setting is None:  # default is used
    ...
    def _get_edge_traces(self) -> List[Union[go.Scatter, go.Scatter3d]]:
        ...
        for (color, width), edges in groups.items():
            _x, _y, _z = [], [], []
            for _v, _u in edges:
                _x += [self.pos[_v][0], self.pos[_u][0], None]
                _y += [self.pos[_v][1], self.pos[_u][1], None]
                if self.is_3d:
                    _z += [self.pos[_v][2], self.pos[_u][2], None]
            ...
            traces += [go.Scatter3d(z=_z, **params) if self.is_3d else go.Scatter(**params)]
